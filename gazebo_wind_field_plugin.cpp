...
void GazeboWindPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  std::cout << "The  is: " << wind_pub_topic_ << std::endl;
  ...
  //update_connection_ = event::Events::ConnectWorldUpdateBegin(std::bind(&GazeboWindPlugin::OnUpdate, this, _1));
...
void GazeboWindPlugin::OnUpdate() {

  // printf("3");
  // Get the current simulation time.
  common::Time now = world_->SimTime().Double();
